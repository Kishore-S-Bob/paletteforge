    ...
    <div className="max-w-5xl w-full flex flex-col gap-16 pb-24">
      ...
      <section className="flex flex-col items-center text-center pt-12">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-5xl font-black tracking-tight text-brand-slate-900">
            Palette<span className="text-brand-rose-500">Forge</span>
          </h1>
        </div>
        <p className="text-xl text-brand-slate-500 max-w-2xl mb-12">
          Beautiful colour palettes in seconds. Export as CSS, Tailwind, or SCSS. 
          Built for modern design workflows.
        </p>
        <div className="bg-white p-8 rounded-3xl shadow-xl shadow-brand-slate-200/50 border border-brand-slate-100 mb-4 scale-110">
      ...
      <section>
        <h2 className="text-xs font-black uppercase tracking-[0.2em] text-brand-slate-400 mb-8 flex items-center gap-4">
          Theory Variants
          <div className="h-px bg-brand-slate-200 flex-grow" />
        </h2>
        <div className="flex flex-col gap-4 bg-white p-4 rounded-3xl border border-brand-slate-100 shadow-sm">
          {palettes.map((p) => (
            <PaletteRow 
              key={p.type} 
              palette={p} 
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2">
          <h2 className="text-xs font-black uppercase tracking-[0.2em] text-brand-slate-400 mb-6 flex items-center gap-4">
            Code Export
            <div className="h-px bg-brand-slate-200 flex-grow" />
          </h2>
          ...
          {FEATURED_PALETTES.map((f) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm font-bold text-brand-slate-700 group-hover:text-brand-rose-500 transition-colors">
                  {f.name}
                </span>
      ...
      <footer className="mt-24 pt-12 border-t border-brand-slate-100 text-center">
        <p className="text-brand-slate-400 text-sm">© 2026 PaletteForge Engine. All rights reserved.</p>
      </footer>
    </div>
  );
}
...
export default function Home() {
  return (
    <main className="flex min-h-screen flex-col items-center p-6 md:p-12 bg-brand-slate-50">
      <Suspense fallback={
